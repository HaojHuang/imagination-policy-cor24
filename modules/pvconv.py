...
class PVConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, resolution, attention=False,
        ...
        # print('with se', with_se, with_se_relu)
        if with_se:
    ...
    def forward(self, inputs):
        ...
        # print('voxel coordinate', voxel_coords.shape, voxel_features.shape)
        voxel_features = self.voxel_layers(voxel_features)

        voxel_features = F.trilinear_devoxelize(voxel_features, voxel_coords, self.resolution, self.training)
        fused_features = voxel_features + self.point_features(features)
        # print('voxel feature',voxel_features.shape, self.point_features(features).shape, self.resolution)
        # import time
        # time.sleep(10)
