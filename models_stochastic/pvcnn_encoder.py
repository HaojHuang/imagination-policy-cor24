...
class PVCNN2Base(nn.Module):
    ...
    def forward(self, inputs, colors=None):
        # print(inputs.shape)
        ...
        temb = torch.ones(inputs.shape[0], 1, inputs.shape[-1]).to(inputs.device)
        # print(temb.shape)
        
        coords = inputs[:, :3, :].contiguous()
        if colors is not None:
            features = torch.cat((inputs[:, :3, :].contiguous(), colors), dim=1)
        else:
