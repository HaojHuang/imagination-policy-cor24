...
def train(args):
    ...
    if args.model =='pvcnn':
        flower = FlowPvcnn(model=PVCNN(use_color=args.use_color), device=args.device, lr=1e-4)
    else:
    ...
    for i in range(args.load, args.n+args.load):
        ...
        if args.tab:
            ...
            # o3d.visualization.draw_geometries([to_o3d_pcd(pa,pa_color),to_o3d_pcd(pb,pb_color)], width=1000, height=800)
        if (i+1)% 100 ==0:
            flower.lr_scheduler.step()
        loss = flower.train_one_step(pairs=pairs,mask=mask,color=x0_color,batch_size=args.point_size, id=data['id'], bi=args.bi,lan_emd=lan_emd)
