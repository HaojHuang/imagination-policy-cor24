...
def test(args):
    ...
    # for i in range(6): 
        ...
        # id = 1
        traj,pdf = flower.sample_ode(p0=p0, mask=mask_input, color=x0_color, sample_ode_steps=1000, id=data['id'], bi=args.bi,lan_emd=lan_emd, scale=0.1)
        ...
        if args.disp:
            ...
            o3d.visualization.draw_geometries([to_o3d_pcd(traj[-1]), mesh_frame], window_name="output: generated pts")
    ...
    print('{}: trans_error {} (cm), rot_error {} (degree)'.format(len(trans_errors), trans_errors_avg, rot_errors_avg))
