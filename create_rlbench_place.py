...
class RLBbench_Place_Dataset(InMemoryDataset):
    def __init__(self, root: str, train: bool = True,
        ...
        self.demos = 10
        super(RLBbench_Place_Dataset,self).__init__(root, transform, pre_transform, pre_filter)
    ...
    def process(self,):
        ...
        for task_name in self.task_list:
            ...
            for fname in sorted(os.listdir(os.path.join('./rlbench_data_processed',task_name,'dict'))):
                ...
                # base pa
                pa = datapoint['base_points'].to(torch.float32)
                pa_color = datapoint['base_colors'].to(torch.float32)
                # downsample pa
                ...
                pa_cano_pose = Transform.from_matrix(datapoint['base_init_matrix']).inverse()
                pa_cano_pose.translation = [0,0,0]
                pa_center_off = Transform(rotation=Rotation.identity(), translation=-pa_mean[0].numpy())
                pa_cano_pose = pa_cano_pose * pa_center_off
                pa = pa_cano_pose.transform_point(pa)
                Ta = np.eye(4)
                Ta =  torch.from_numpy(Ta).to(torch.float32)
                
                # mesh_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1, origin=[0, 0, 0])
                # o3d.visualization.draw_geometries([mesh_frame, to_o3d_pcd(pa, pa_color),])
                ...
                preplace_pose = Transform.from_matrix(datapoint['preplace_pose'])
                place_pose = Transform.from_matrix(datapoint['place_pose'])
                pick_pose = Transform.from_matrix(datapoint['pick_pose'])
                Tb_pre = pa_cano_pose * (preplace_pose* (pick_pose.inverse())) * pb_center_off
                Tb_place = pa_cano_pose * (place_pose* (pick_pose.inverse())) * pb_center_off
                
                # mesh_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1, origin=[0, 0, 0])
                # o3d.visualization.draw_geometries([mesh_frame, to_o3d_pcd(pa, pa_color), to_o3d_pcd(pb,pb_color).transform(Tb_place.as_matrix())])
                # print(pa.shape, pb.shape)
                # preplace data
                pb_pre_transformed = to_o3d_pcd(pb,pb_color).transform(Tb_pre.as_matrix())
                pab_pre = torch.cat((torch.from_numpy(pa).float(),torch.from_numpy(np.asarray(pb_pre_transformed.points)).to(torch.float)),dim=0)
                pab_pre_color = torch.cat((pa_color,torch.from_numpy(np.asarray(pb_pre_transformed.colors)).to(torch.float)),dim=0)
                lan_pre = datapoint['lan_preplace']
                
                # o3d.visualization.draw_geometries([mesh_frame, to_o3d_pcd(pab_pre, pab_pre_color),to_o3d_pcd(pb)])
                
                
                # place data
                pb_place_transformed = to_o3d_pcd(pb,pb_color).transform(Tb_place.as_matrix())
                pab_place = torch.cat((torch.from_numpy(pa).float(),torch.from_numpy(np.asarray(pb_place_transformed.points)).to(torch.float)),dim=0)
                pab_place_color = torch.cat((pa_color,torch.from_numpy(np.asarray(pb_place_transformed.colors)).to(torch.float)),dim=0)
                lan_place = datapoint['lan_place']
                
                # o3d.visualization.draw_geometries([mesh_frame, to_o3d_pcd(pab_place, pab_place_color)])
                
                # print(lan)
                Tb_pre = torch.from_numpy(Tb_pre.as_matrix()).float()
                Tb_place = torch.from_numpy(Tb_place.as_matrix()).float()
                pa = torch.from_numpy(pa).float()
                
                
                data_pre = Data(pa=pa,pa_color=pa_color,pa_ds=pa, pa_ds_color =pa_color,Ta=Ta, 
                                pb=pb, pb_color=pb_color, pb_ds=pb, pb_ds_color =pb_color,Tb=Tb_pre,
                                pab = pab_pre, pab_color=pab_pre_color, pab_ds = pab_pre, pab_ds_color=pab_pre_color,
                                lan= lan_pre, id='pre_place')
                data_list.append(data_pre)
                data_place = Data(pa=pa,pa_color=pa_color,pa_ds=pa,pa_ds_color=pa_color,Ta=Ta,
                                  pb=pb, pb_color=pb_color, pb_ds=pb, pb_ds_color=pb_color, Tb=Tb_place,
                                  pab = pab_place, pab_color=pab_place_color, pab_ds = pab_place, pab_ds_color=pab_place_color,
                                  lan=lan_place, id='place')
                data_list.append(data_place)
...
# tr_dataset = RLBbench_Place_Dataset(root='./rlbench_data_processed/place', pre_transform=Compose([addLanEmd(),]), transform=augData(aug1=True, aug2=True, randsample=True), train=True)
