...
class TaskEnvironment(object):
    ...
    def step(self, action, test_mode=False) -> (Observation, int, bool):
        # returns observation, reward, done, info
        if not self._reset_called:
            raise RuntimeError(
                "Call 'reset' before calling 'step' on a task.")

        #get away from breaking the code
        if test_mode:
            try:
    ...
    def get_demos(self, amount: int, live_demos: bool = False,
        ...
        if not live_demos:
            ...
            demos = self._get_live_demos(
                amount, callable_each_step, max_attempts, home_time=home_time)
    ...
    def _get_live_demos(self, amount: int,
        ...
        for i in range(amount):
            ...
            while attempts > 0:
                ...
                self.reset()
                try:
                    base_matrix = self._task.get_base().get_matrix()
                    obj_matrix = self._task._graspable_objects[0].get_matrix()
                    # print(base_matrix)
                    # print(obj_matrix)

                    print('attempt ', attempts)
                    demo = self._scene.get_demo(
                        callable_each_step=callable_each_step,home_time=home_time)
                    demo.random_seed = random_seed
                    demo.base_matrix = base_matrix
                    demo.obj_matrix = obj_matrix
                    demos.append(demo)
                    break
