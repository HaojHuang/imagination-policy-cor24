...
class Scene(object):
    ...
    def get_demo(self, record: bool = True,
                 ...
                 randomly_place: bool = True, home_time=None) -> Demo:
        """Returns a demo (list of observations)"""
        # add home time manually here
        if self.task.get_name()=='stack_cups':
            home_time =5
        ...
        #print('a')
        pose0 = self.robot.arm.get_tip().get_pose()
        ...
        while True:
            ...
            for i, point in enumerate(waypoints):
                ...
                #print('====', i, home_time)
                if home_time is not None and (i+1) % home_time==0:
                    colliding_shapes = [s for s in self.pyrep.get_objects_in_tree(object_type=ObjectType.SHAPE)
                    ...
                    try:
                        ...
                        path = self.robot.arm.get_path(position0,
                                                       quaternion=quat0,
                        ...
                        [s.set_collidable(True) for s in colliding_shapes]
                        #print('move home down')
                    except ConfigurationPathError as e:
                        [s.set_collidable(True) for s in colliding_shapes]
                        raise DemoError(
                            'Could not get a path home.',
                            self.task) from e
                    path.visualize()
                    done = False
                    success = False
