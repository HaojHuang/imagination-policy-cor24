...
def get_stored_demos(amount: int, image_paths: bool, dataset_root: str,
    ...
    for example in selected_examples:
        ...
        if not (num_steps == len(listdir(l_sh_rgb_f)) == len(
            ...
            # print(num_steps)
            # print(len(listdir(l_sh_rgb_f)))
            # print(len(listdir(oh_depth_f)))
...
def get_stored_demo(data_path, index,init_matrix=False):
    ...
    for i in range(num_steps):
        ...
        obs[i].front_mask = np.array(
            Image.open(os.path.join(episode_path, '%s_%s' % (CAMERA_FRONT, 'mask'), IMAGE_FORMAT % i)))
        obs[i].left_shoulder_mask = np.array(
            Image.open(os.path.join(episode_path, '%s_%s' % (CAMERA_LS, 'mask'), IMAGE_FORMAT % i)))
        obs[i].right_shoulder_mask = np.array(
            Image.open(os.path.join(episode_path, '%s_%s' % (CAMERA_RS, 'mask'), IMAGE_FORMAT % i)))
        obs[i].wrist_mask = np.array(
            Image.open(os.path.join(episode_path, '%s_%s' % (CAMERA_WRIST, 'mask'), IMAGE_FORMAT % i)))
    ...
    if init_matrix:
        return obs, base_matrix, obj_matrix
    else:
        return obs